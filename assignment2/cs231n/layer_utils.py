...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_params):
    """
    Convenience layer that performs an affince transform, then a batchnorm,
    then a ReLU
    Inputs:
    - x, w, b: see affine_relu_forward():
    - gamma, beta: arguments to batchnorm.
    - bn_params, see fc_net.py
    Returns a tuple of:
    - out: Output from the ReLU
    - cache: Object to give to the backward pass
    """
