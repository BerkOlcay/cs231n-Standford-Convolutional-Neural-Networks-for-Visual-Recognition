...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # variables w1, w2, b1, b2, layer1scores, hidden_scores, scores, loss
        # gradients dw1, dw2, db1, db2, dlayer1scores, dhidden_scores, dscores, dloss
        # dscores will be computed, dloss/dloss...
        # so, find dw1, dw2, db1, db2, dlayer1scores, dhidden_scores
        ...
        # backpropate the gradient to the parameters (W1, W2, b1, b2)
        ### scores = W2 * hidden_scores + b2
        # dScores/dW2 = d(W2 * hidden_scores)/dW2 + db2/dW2 = 1
        #1# dScores/dhidden = hidden_scores
        # dScores/dhidden = d(W2 * hidden_scores)/dhidden + db2/dhidden
        #2# dScores/dhidden = W2
        # dScores/db2 = d(W2 * hidden_scores)/db2 + db2/db2
        #3# dScores/db2 = 1
        
        # dLoss/dW2 = dLoss/dscores * dScores/dW2
        #4# dLoss/dW2 = dscores * h1
        grads['W2'] = hidden_scores.T.dot(dscores)
        # dLoss/db2 = dLoss/dscores * dScores/db2
        #5# dLoss/db2 = dscores * 1
        grads['b2'] = dscores.sum(axis=0)
        # dLoss/dhidden = dLoss/dscores * dScores/dhidden
        #6# dhidden = dscores * W2
        dhidden = dscores.dot(W2.T)
        
        # dLoss/dlayer1 = dLoss/dhidden * dhidden/dlayer1
        ### hidden_scores = np.maximum(layer1scores, 0)
        #7# dhidden/dlayer1 = 1 if layer1scores > 0, 0 otherwise
        dhidden_dlayer1 = np.zeros_like(layer1scores)
        dhidden_dlayer1[layer1scores > 0] = 1
        #8# dlayer1 = dhidden * dhidden/dLayer1Scores
        dlayer1 = dhidden * dhidden_dlayer1
        
        
        ### h1 = W1 * X + b1
        # dh1/dW1 = d(W1 * X)/dW1 + db1/dW1 = 1
        #9# dh1/dW1 = X 
        # dh1/db1 = d(W1 * X)/db1 + db1/db1 = 1
        #10# dh1/db1 = 1 
    ...
    def predict(self, X):
        ...
        layer1scores = X.dot(self.params['W1']) + self.params['b1']
        hidden_scores = np.maximum(layer1scores, 0)
        # calculate output layer scores
